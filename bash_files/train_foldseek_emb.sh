...
while getopts "d:t:F:T:e:E:vo:p:" args; do
    case $args in
        d) DEVICE=${OPTARG}
        ;;
        t) TRAIN=${OPTARG}
...
# ./train_foldseek_emb.sh -o tt_fseek_embed -v -d 2

FOLDSEEK_CMD="--foldseek_fasta ${FOLDSEEK_FASTA} --foldseek_vocab ${FOLDSEEK_VOCAB} --fseek_lang_emb 25"

if [ ! -d ${OUTPUT_FOLDER} ]; then mkdir -p $OUTPUT_FOLDER; fi



dscript fseek --train $TRAIN --test $TEST --embedding $EMBEDDING $TOPSY_TURVY \
              ...
              --kernel-width 7 --dropout-p 0.2 --projection-dim 75 \
