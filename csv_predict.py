...
if __name__ == '__main__':
    ...
    parser.add_argument('csv', help='Candidate protein pairs to predict')
    parser.add_argument('fasta', help='Protein sequences in .fasta format')
    parser.add_argument('model', help='path to model')
    parser.add_argument('outfile', help='file for predictions')
    parser.add_argument('-d', '--device', type=int, default=-1, help='compute device to use')
    parser.add_argument('--pos_threshold', type=float, default=THRESH, help='Matches predicted above [pos_threshold] are written to the .positive file')
    parser.add_argument('--embeddings', help='Use pre-computed embeddings')
...
if embPath: precomputedEmbeddings = True
else: precomputedEmbeddings = False
...
if DO_WARN and (tot > WARN_THRESH):
    contin = input('! Number of predictions ({}) exceeds threshold ({}). Do you want to continue? [y/n] '.format(int(tot), WARN_THRESH))
    if contin.lower() != 'y':
        sys.exit(0)
