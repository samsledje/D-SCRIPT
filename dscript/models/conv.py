...
class ProteinConv(nn.Module):
    """
    """
    def __init__(self, no_filters, no_dims, no_channels, window_size, stride = 1, dropout_p = 0.2, activation = "tanh"):
        ...
        self.W = nn.Parameter(torch.randn(no_filters, no_channels, no_dims, window_size, dtype = torch.float32))
    ...
    def attention(self, p1folds, p2folds):
        ...
        H1_ = p1folds.size(1)
        H2_ = p2folds.size(1)
        p1folds = torch.sum(p1folds * self.W.unsqueeze(0).unsqueeze(0), dim = [3, 5]) # N x H1' x 1 x C x D x window => N x H1' x F x D
        p2folds = torch.sum(p2folds * self.W.unsqueeze(0).unsqueeze(0), dim = [3, 5]) # N x H2' x 1 x C x D x window => N x H2' x F x D
        
        p1folds = torch.transpose(p1folds, 1, 2) # => N x F x H1' x D
        p2folds = torch.transpose(p2folds, 1, 2) # => N x F x H2' x D
        
        att = torch.matmul(p1folds, torch.transpose(p2folds, 2, 3)) / torch.sqrt(H1_ * H2_) # => N x F x H1' x H2'
        p1folds = torch.matmul(F.softmax(att, dim = 3), p2folds) # => N x F x H1' x D
        p2folds = torch.matmul(F.softmax(torch.transpose(att, 2, 2), dim = 3), p1folds) # => N x F x H2' x D
    ...
    def forward(self, prot1, prot2):
        ...
        assert (N1, C1, D1) == (N2, C2, D2) ## Only difference between the proteins should be their sequence length
        
        # unfold prot1 on the third dimension
        p1folds = prot1.unfold(1, self.window, self.stride).unsqueeze(2) # N x H1' x 1 x C x D x window
        p2folds = prot2.unfold(1, self.window, self.stride).unsqueeze(2) # N x H2' x 1 x C x D x window
...
#         p2out  = torch.sum(p2folds * p1sum, dim = [3, 5]) # N x H1' x F x C x D1 x window => N x H1' x F x D
        return self.drop1(self.activation(p1folds)), self.drop2(self.activation(p2folds))
