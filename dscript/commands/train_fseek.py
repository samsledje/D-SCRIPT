...
from ..models.fseek_interaction import ModelInteractionFseek


class TrainFArguments(NamedTuple):
    cmd: str
    device: int
    train: str
    test: str
    ...
    seed: Optional[int]
    fseek_lang_emb: Optional[int]
    foldseek_fasta: Optional[str]
    foldseek_vocab: Optional[str]
    func: Callable[[TrainFArguments], None]
...
def add_args(parser):
    ...
    foldseek_grp.add_argument(
        ...
        default=25,
        type=int,
        help="Foldseek lang embedding size",
    )
    foldseek_grp.add_argument(
        "--foldseek_fasta",
...
def get_foldseek(n0, size_n0, fold_record, fold_vocab):
    """
    ...
    if n0 in fold_record:
        ...
        foldseek_enc = torch.zeros(size_n0, dtype=torch.long)
        for i, a in enumerate(fold_seq):
            assert a in fold_vocab
            foldseek_enc[i] = fold_vocab[a]
        return foldseek_enc
    else:
        # all `X`
        return 20 * torch.ones(size_n0, dtype=torch.float32)
...
def predict_cmap_interaction(
    ...
    :type model: dscript.models.interaction.ModelInteractionFseek
    ...
    for i in range(b):
        ...
        cm, ph = model.map_predict(z_a, z_b, f_a, f_b)
        p_hat.append(ph)
...
def predict_interaction(
    model, n0, n1, tensors, use_cuda, fold_record, fold_vocab
):
    """
...
def train_model(args, output):
    ...
    if args.checkpoint is None:
        ...
        fseek_lang_emb = args.fseek_lang_emb
        ...
        proj_dim = projection_dim + fseek_lang_emb
